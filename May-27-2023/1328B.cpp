...
void solve() {
    ...
    string ans = "a";
    for(int i = 1; i < n; i++){
        pref.pb(pref[pref.size() - 1] + i);
        ans += "a";
    }
    int x = (n - (lower_bound(all(pref), k) - pref.begin()) - 1);
    int y = n - (k - ((lower_bound(all(pref), k) - pref.begin()) * (lower_bound(all(pref), k) - pref.begin() - 1) / 2));
    ans[x] = 'b';
    ans[y] = 'b';
    cout << ans << endl;
}
